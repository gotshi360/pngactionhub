...
def _is_enabled_for_role(ctx) -> bool:
    try:
        ...
        role_owner  = shared.get("rs_role_owner",  True)
        role_admin  = shared.get("rs_role_admin",  True)
        role_member = shared.get("rs_role_member", True)
...
def on_is_action_enabled(path, type, ctx):
    try:
        shared = aps.SharedSettings(ctx.workspace_id, SETTINGS_NAME)
        if not shared.get("show_repo_settings", True):
            return False
